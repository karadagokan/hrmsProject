...
public interface AdvertisementDao extends JpaRepository<Advertisement, Integer> {
    ...
    @Query(value = "SELECT new com.karadag.hrmsProject.entities.dtos.AdvertisementDetail(e.companyName,p.positionName,a.numberOfOpenPosition,a.realeseDate,a.applicationDeadline) FROM Advertisement as a LEFT JOIN a.employer as e left join a.jobPosition as p where a.jobStatus=true ")
    List<AdvertisementDetail> getAllJobAdvertisement();

    @Query("SELECT new com.karadag.hrmsProject.entities.dtos.AdvertisementDetail(e.companyName,p.positionName,a.numberOfOpenPosition,a.realeseDate,a.applicationDeadline) FROM Advertisement as a LEFT JOIN a.employer as e left join a.jobPosition as p where a.jobStatus=true and e.companyName=:companyName")
